	...
	<li class="comment">
		...
		<?php echo get_avatar($comment, $size = '50'); ?>
		    ...
		    <div class="comment-meta">
		        ...
		        <p class="date"><?php printf(__('%1$s at %2$s', 'nash'), get_comment_date(),  get_comment_time()) ?></p>
		    </div>
		...
		<?php if ($comment->comment_approved == '0') : ?>
			<em class="comment-moderate"><?php _e('Your comment is awaiting moderation.', 'nash') ?></em>
			<br />
		<?php endif; ?>
		
		<?php edit_comment_link(__('(Edit)', 'nash'),'  ','') ?>
...
function gt_comment_form($form_options) {
    ...
    $fields = array(
	    ...
	    '<input id="author" name="author" type="text" size="30" placeholder="' . __( 'Your Name (required)', 'nash' ) . '" />' .
	    '</p>',
	
	    'email' =>
	    '<p class="comment-form-email">' .
	    '<input id="email" name="email" type="text" size="30" placeholder="' . __( 'Your Email (will not be published)', 'nash' ) . '" />' .
	    '</p>',
	
	    'url' =>
	    '<p class="comment-form-url">'  .
	    '<input name="url" size="30" id="url" type="text" placeholder="' . __( 'Your Website (optional)', 'nash' ) . '" />' .
	    '</p>',
    ...
    $form_options = array(
        ...
        '<textarea name="comment" id="comment" aria-required="true" rows="8" cols="45" placeholder="' . __( 'Please leave your comment...', 'nash' ) . '"></textarea>' .
        '</p>',

        'must_log_in' =>
        '<p class="must-log-in">' .
        sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'nash' ),
            wp_login_url( apply_filters( 'the_permalink', get_permalink() ) ) ) .
        '</p>',

        'logged_in_as' =>
        '<p class="logged-in-as">' .
        sprintf( __( 'You are currently logged in<a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'nash' ),
        ...
        'id_submit' => 'submit',
        'title_reply' => __( 'Please leave a Comment', 'nash' ),
        'title_reply_to' => __( 'Leave a Reply to %s', 'nash' ),
        'cancel_reply_link' => __( 'Cancel reply', 'nash' ),
        'label_submit' => __( 'Post Comment', 'nash' ),
    );
...
<ul class="pager">
		<?php previous_post_link( '<li class="previous">%link</li>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'nash' ) . '</span> %title' ); ?>
		<?php next_post_link( '<li class="next">%link</li>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'nash' ) . '</span>' ); ?>
</ul>
