...
function team_register() {
    $labels = array(
        ...
        'add_new_item' => __('Add New Team Member', 'nash'),
        'edit_item' => __('Edit Team Member', 'nash'),
        'new_item' => __('New Team Member', 'nash'),
        'view_item' => __('View Team Member', 'nash'),
        'search_items' => __('Search Team Members', 'nash'),
...
function team_help_text($contextual_help, $screen_id, $screen) {
    if ('team' == $screen->id) {
        $contextual_help =
        '<h3>' . __('Things to remember when adding a Team Member:', 'nash') . '</h3>' .
        '<ul>' .
        '<li>' . __('Add your Team Member name. (ie; Lana Del Rey).', 'nash') . '</li>' .
        '<li>' . __('Add a short description (Bio) for your member.', 'nash') . '</li>' .
        '<li>' . __('Add a featured image (headshot possibly) to appear above your team member.', 'nash') . '</li>' .
        '<li>' . __('Add a Job Title for your member. (ie; The Boss or Managing Director).', 'nash') . '</li>' .
        '<li>' . __('Add any Social Networks your Team Member belongs to. (ie; Twitter or Linkedin).', 'nash') . '</li>' .
        '</ul>';
    }
    elseif ('edit-team' == $screen->id) {
        $contextual_help = '<p>' . __('A list of all team members appear below. To edit a member, click on the Team Member name.', 'nash') . '</p>';
    }
...
function team_image_box() {
	...
	add_meta_box('postimagediv', __('Team Member Image', 'nash'), 'post_thumbnail_meta_box', 'team', 'side', 'low');
}
...
?>
    ...
    <h2><?php _e('Sort Team Members', 'nash'); ?> <img src="<?php echo home_url(); ?>/wp-admin/images/loading.gif" id="loading-animation" /></h2>
    <p><?php _e('Click, drag, re-order. Repeat as neccessary. Team Member at the top will appear first on your page.', 'nash'); ?></p>
    <ul id="post-list">
