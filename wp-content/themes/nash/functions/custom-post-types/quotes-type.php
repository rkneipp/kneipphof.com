...
function quotes_register() {
    $labels = array(
        ...
        'add_new_item' => __('Add New Quote', 'nash'),
        'edit_item' => __('Edit Quote Item', 'nash'),
        'new_item' => __('New Quote Item', 'nash'),
        'view_item' => __('View Quote Item', 'nash'),
        'search_items' => __('Search Quote Items', 'nash'),
...
function quotes_help_text($contextual_help, $screen_id, $screen) {
    if ('quotes' == $screen->id) {
        $contextual_help =
        '<h3>' . __('Things to remember when adding a Quote:', 'nash') . '</h3>' .
        '<ul>' .
        '<li>' . __('Give the Quote a reference name. This will not appear on your site, and is just for reference purposes (ie; Quote from Jules Verne).', 'nash') . '</li>' .
        '<li>' . __('Enter a short quote or testimonial.', 'nash') . '</li>' .
        '<li>' . __('Enter the name of the quote author.', 'nash') . '</li>' .
        '</ul>';
    }
    elseif ('edit-quotes' == $screen->id) {
        $contextual_help = '<p>' . __('A list of all quotes appear below. To edit a quote, click on the quote Reference Name.', 'nash') . '</p>';
    }
...
?>
    ...
    <h2><?php _e('Sort Quote Items', 'nash'); ?> <img src="<?php echo home_url(); ?>/wp-admin/images/loading.gif" id="loading-animation" /></h2>
    <p><?php _e('Click, drag, re-order. Repeat as neccessary. Quote item at the top will appear first on your page.', 'nash'); ?></p>
    <ul id="post-list">
